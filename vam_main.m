...
disp(['scNum = ',num2str(L1.scNum),'; PRN = ',num2str(L1.prn_code),'; ddm_index = ',num2str(L1.ddm_index),'; sample_index = ',num2str(L1.index)]);
...
% figure;imagesc(BKG.LON_vec(splon_index-k:splon_index+k),BKG.LAT_vec(splat_index-k:splat_index+k),...
% WS(splon_index-k:splon_index+k,splat_index-k:splat_index+k)');set(gca,'YDir','normal');
...
diff = max(diff1,diff2); % different from the definition in QJ paper
